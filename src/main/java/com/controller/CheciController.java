    ...
    */
    ...
    public R save(@RequestBody CheciEntity checi, HttpServletRequest request){
        ...
        Wrapper<CheciEntity> queryWrapper = new EntityWrapper<CheciEntity>()
            ...
            .eq("checi_fuzeren", checi.getCheciFuzeren())
            .eq("checi_address", checi.getCheciAddress())
            .eq("checi_kucun_number", checi.getCheciKucunNumber())
            ;

        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        CheciEntity checiEntity = checiService.selectOne(queryWrapper);
        if(checiEntity==null){
            checi.setInsertTime(new Date());
...
//                            checiEntity.setCheciTypes(Integer.valueOf(data.get(0)));   //车票类型 要改的
//                            checiEntity.setCheciPhoto("");//详情和图片
//                            checiEntity.setCheciFuzeren(data.get(0));                    //起始站 要改的
//                            checiEntity.setCheciAddress(data.get(0));                    //终点站 要改的
//                            checiEntity.setCheciKucunNumber(Integer.valueOf(data.get(0)));   //车票余量 要改的
//                            checiEntity.setCheciNewMoney(data.get(0));                    //车票价格 要改的
//                            checiEntity.setCheciTime(sdf.parse(data.get(0)));          //发车时间 要改的
//                            checiEntity.setCheciContent("");//详情和图片
//                            checiEntity.setInsertTime(date);//时间
