...
//                            checiOrderEntity.setCheciId(Integer.valueOf(data.get(0)));   //车票 要改的
//                            checiOrderEntity.setYonghuId(Integer.valueOf(data.get(0)));   //用户 要改的
//                            checiOrderEntity.setBuyNumber(Integer.valueOf(data.get(0)));   //购买数量 要改的
    ...
    */
    ...
    public R add(@RequestBody CheciOrderEntity checiOrder, HttpServletRequest request){
        logger.debug("add方法:,,Controller:{},,checiOrder:{}",this.getClass().getName(),checiOrder.toString());
            ...
            if(checiEntity == null){
                return R.error(511,"查不到该车票信息");
            }
            // Double checiNewMoney = checiEntity.getCheciNewMoney();

            if(false){
            }
            else if((checiEntity.getCheciKucunNumber() -checiOrder.getBuyNumber())<0){
                return R.error(511,"购买数量不能大于库存数量");
            }
            else if(checiEntity.getCheciNewMoney() == null){
                return R.error(511,"车票信息价格不能为空");
            }
            ...
            double balance = yonghuEntity.getNewMoney() - checiEntity.getCheciNewMoney()*checiOrder.getBuyNumber();//余额
            if(balance<0)
                return R.error(511,"余额不够支付");
            checiOrder.setCheciOrderTypes(1); //设置订单状态为已支付
            checiOrder.setCheciOrderTruePrice(checiEntity.getCheciNewMoney()*checiOrder.getBuyNumber()); //设置实付价格
            checiOrder.setYonghuId(userId); //设置订单支付人id
            checiOrder.setCheciOrderUuidNumber(String.valueOf(new Date().getTime()));
            checiOrder.setInsertTime(new Date());
            checiOrder.setCreateTime(new Date());
                checiEntity.setCheciKucunNumber( checiEntity.getCheciKucunNumber() -checiOrder.getBuyNumber());
                checiService.updateById(checiEntity);
    ...
    */
    ...
    public R refund(Integer id, HttpServletRequest request){
        ...
        String role = String.valueOf(request.getSession().getAttribute("role"));
            ...
            CheciOrderEntity checiOrder = checiOrderService.selectById(id);
            Integer buyNumber = checiOrder.getBuyNumber();
            ...
            checiEntity.setCheciKucunNumber(checiEntity.getCheciKucunNumber() + buyNumber);
            ...
            checiService.updateById(checiEntity);//更新订单中车票信息的信息
            return R.ok();
    }
